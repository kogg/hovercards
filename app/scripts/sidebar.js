...
module.exports = function sidebar() {
    ...
    var minimizer = $('<div></div>')
        ...
        .click(function() {
            ...
            $.analytics('send', 'event', 'sidebar ' + (obj.hasClass(EXTENSION_ID + '-sidebar-minimized') ? 'minimized' : 'unminimized'), 'minimizer');
        });
    ...
    function sendMessage(message) {
        switch (message.msg) {
            case EXTENSION_ID + '-load':
                ...
                if (message.by !== 'back') {
                    if (_.chain(identity_history).last().isEqual(message.identity).value()) {
                        if (showing) {
                            ...
                            $.analytics('send', 'event', 'sidebar activated (same)', message.by, label);
                ...
                label = (message.identity.type === 'url') ? 'url' : message.identity.api + ' ' + message.identity.type;
                $.analytics('send', 'event', 'sidebar activated', message.by, label);
                break;
            case EXTENSION_ID + '-hide':
                ...
                $.analytics('send', 'event', 'sidebar deactivated', message.by);
                break;
        }
