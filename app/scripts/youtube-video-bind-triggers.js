    ...
    function($, hoverTrigger, youtubeVideoBindTriggersOnPlayer) {
        return {
            on: function youtubeVideoBindTriggersOn(body, docURL) {
                ...
                hoverTrigger.on(body, 'youtube', 'youtube-video', 'a[href*="youtube.com/watch"]', function() {
                    return purl($(this).attr('href')).param('v');
                });
                hoverTrigger.on(body, 'youtube', 'youtube-video', 'a[data-href*="youtube.com/watch"]', function() {
                    return purl($(this).data('href')).param('v');
                });
                if (doc.attr('host') === 'www.youtube.com') {
                    hoverTrigger.on(body, 'youtube', 'youtube-video', 'a[href*="/watch"]', function() {
                        return purl($(this).attr('href')).param('v');
                    });
                ...
                hoverTrigger.on(body, 'youtube', 'youtube-video', 'a[href*="youtu.be/"]', function() {
                    return purl($(this).prop('href')).segment(-1);
                });
                hoverTrigger.on(body, 'youtube', 'youtube-video', 'embed[src*="youtube.com/v/"]', function() {
                    return purl($(this).prop('src')).segment(-1);
                });
                hoverTrigger.on(body, 'youtube', 'youtube-video', 'object[data*="youtube.com/v/"]', function() {
