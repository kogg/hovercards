...
module.exports = function(params) {
	...
	function soundcloud(endpoint, args, callback) {
		request({ url:  'https://api.soundcloud.com' + endpoint,
		          qs:   _.defaults({ client_id: params.key }, args),
		          json: true },
	...
	function user_to_account(user) {
		...
		return !_.isEmpty(user) && _.pick({ api:   'soundcloud',
		                                    type:  'account',
		                                    id:    _.result(user, 'permalink'),
		                                    name:  _.result(user, 'username'),
		                                    image: !_.isEmpty(user_artwork_url) &&
		                                           !user_artwork_url.match(/default_avatar_large/) &&
		                                           { small:  user_artwork_url,
		                                             medium: user_artwork_url.replace('-large', '-t300x300'),
		                                             large:  user_artwork_url.replace('-large', '-t500x500') } }, _.negate(_.isEmpty));
	}

	function post_to_content(post) {
		var artwork_url = _.result(post, 'artwork_url', '');
		return !_.isEmpty(post) && _.pick({ api:     'soundcloud',
		                                    type:    'content',
		                                    id:      _.result(post, 'permalink'),
		                                    as:      (_.result(post, 'kind') === 'playlist') && 'playlist',
		                                    name:    _.result(post, 'title'),
		                                    date:    Date.parse(_.result(post, 'created_at')),
		                                    stats:   _.result(post, 'kind') === 'playlist' ? { content: Number(_.result(post, 'track_count')) } :
		                                                                                     { likes:    Number(_.result(post, 'favoritings_count')),
		                                                                                       views:    Number(_.result(post, 'playback_count')),
		                                                                                       comments: Number(_.result(post, 'comment_count')) },
		                                    image:   !artwork_url.match(/default_avatar_large/) &&
		                                             { small:  artwork_url,
		                                               medium: artwork_url.replace('-large', '-t300x300'),
		                                               large:  artwork_url.replace('-large', '-t500x500') },
		                                    account: user_to_account(post.user) }, _.somePredicate(_.isNumber, _.negate(_.isEmpty)));
	}
	...
	api.content = function(args, callback) {
		...
		model.resolve({ url: urls.print(_.defaults({ api: 'soundcloud', type: 'content' }, args)) }, null, usage, function(err, post) {
			...
			callback(null, _.chain(post_to_content(post))
			                .extend({ text:        autolinker.link((_.result(post, 'description') || '')
			                                                         .replace(/\n+$/, '')
			                                                         .replace(/\n/g, '<br>')),
			                          // Playlists and certain tracks can't be commented on.
			                          // We give them an empty discussion so the extension
			                          // doesn't try to load it
			                          discussions: !_.result(post, 'commentable') && [{ api:           'soundcloud',
			                                                                            type:          'discussion',
			                                                                            id:            _.result(post, 'permalink'),
			                                                                            uncommentable: true }] })
	...
	api.discussion = function(args, callback) {
		...
		async.waterfall([
			...
			callback(null, _.pick({ api:      'soundcloud',
			                        type:     'discussion',
			                        id:       args.id,
			                        comments: _.chain(comments)
			                                   .first(config.counts.listed)
			                                   .map(function(comment) {
			                                       return _.pick({ api:         'soundcloud',
			                                                       type:        'comment',
			                                                       ...
			                                                       account:     user_to_account(_.result(comment, 'user')) }, _.somePredicate(_.isNumber, _.negate(_.isEmpty)));
			                                   })
			                                   .reject(_.isEmpty)
	...
	api.account = function(args, callback) {
		...
		async.auto({
			...
			var text = autolinker.link((_.result(results.user, 'description') || '')
			                             .replace(/\n+$/, '')
			                             .replace(/\n/g, '<br>'));

			callback(null, _.chain(user_to_account(results.user))
			                .extend({ text:     text,
			                          stats:    { content:   Number(_.result(results.user, 'track_count')),
			                                      followers: Number(_.result(results.user, 'followers_count')),
			                                      following: Number(_.result(results.user, 'followings_count')) },
			                          accounts: _.chain(results.web_profiles)
			                                     .pluck('url')
			                                     .union([_.result(results.user, 'website')])
			                                     .union(_.invoke(text.match(/href="[^"]+"/g), 'slice', 6, -1))
			                                     .map(urls.parse)
			                                     .where({ type: 'account' })
			                                     .uniq(false, function(account) { return _.compact([account.api, account.id, account.as]).join('/'); })
			                                     .value() })
			                .pick(_.somePredicate(_.isNumber, _.negate(_.isEmpty)))
			                .value(), usage);
		});
	...
	api.account_content = function(args, callback) {
		...
		async.auto({
			...
			callback(null, _.pick({ api:     'soundcloud',
			                        ...
			                        id:      args.id,
			                        content: _.chain(results.user_tracks)
			                                  .union(results.user_playlists)
			                                  .sortBy(function(post) { return -Date.parse(post.created_at); })
