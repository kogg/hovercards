...
module.exports = function(params) {
	...
	api.account_content = function(args) {
		...
		return model.channel(_.pick(args, 'id'), _.pick(args, 'device_id'), usage)
			.then(function(channel) {
				return model.playlist_items({ id: _.chain(channel) .result('contentDetails') .result('relatedPlaylists') .result('uploads') .value() }, _.pick(args, 'device_id'), usage);
			})
			.then(function(playlist_items) {
				return _.pick({ api:     'youtube', type:    'account_content', id:      _.result(args, 'id'), content: _.chain(playlist_items) .map(function(video) {
					return _.extend(video_to_content(video), { id: _.chain(video) .result('snippet') .result('resourceId') .result('videoId') .value() }); }) .reject(_.isEmpty) .value() }, _.negate(_.isEmpty));
			});
	};
	...
	model.playlist_items = function(args, args_not_cached, usage) {
		usage['youtube-quota'] += 3;
		var youtube = _.sample(youtubes);

		return promisify(youtube.playlistItems.list.bind(youtube.playlistItems), { multiArgs: true })({
			part:       'snippet',
			playlistId: args.id,
			quotaUser:  args_not_cached.device_id || params.device_id,
